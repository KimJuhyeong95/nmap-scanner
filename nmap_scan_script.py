...
def run_nmap(ip_or_domain, ports, args="-sS"):
    ...
    command = ["nmap"] + args.split() + ["-p", ports, ip_or_domain]
...
def extract_services(nmap_output):
    ...
    for line in nmap_output.splitlines():
        if re.search(r"\d+/tcp\s+open\s+\S+", line):
            parts = line.split()
            if len(parts) >= 4:
                port = parts[0]
                service = parts[2]
...
def main():
    ...
    while True:
        ...
        while True:
            ports = input("스캔할 포트 (예: 1-65535 또는 22,80,443): ").strip()
            if validate_ports(ports):
                break
            print("[!] 유효하지 않은 포트 입력입니다. 0~65535 범위의 포트 번호를 입력하세요.")

        mode = input("스캔 모드 선택 (fast / service / full): ").strip().lower()
